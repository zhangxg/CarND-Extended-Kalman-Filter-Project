...
void FusionEKF::ProcessMeasurement(const MeasurementPackage &measurement_pack) {
  ...
  if (!is_initialized_) {
    ...
    // just give an arbitrary value, smaller values mean more confidence.
    ...
    if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
      ...
      */
      ...
      /*
       * should convert the polor values to cartesian values, includ p and x,
       * but don't know how to calculate the vx and vy fro, ro_dot, just get the
       * px and py, and leave the vx, vy both zero.
       * */
  ...
  */
  ...
  if (measurement_pack.sensor_type_ == MeasurementPackage::RADAR) {
    // Radar updates
    ekf_.R_ = R_radar_;
    ekf_.H_ = tools.CalculateJacobian(ekf_.x_); // 3x4
